...
def update_eink(time_str, o2_str, rh_str, temp_str, full=False):
    ...
    if full:
        # On full refresh (init or after disconnect), only show first O₂ value
        ...
        # Partial update: show all fields
...
def display_disconnected():
    epd.init(epd.FULL_UPDATE)
    img = Image.new("1", (land_w, land_h), 0)  # black bg
    draw = ImageDraw.Draw(img)
...
def monitor():
    ...
    # Initial connect & first display (full)
    while True:
        try:
            ...
            update_eink(ts0[11:16],f"O₂: {o2_i:.1f}%",f"RH: {rh_i:.1f}%",f"Temp: {t_i:.1f}°C", full=True)
            connected=True
            break
        except Exception as e:
    ...
    while True:
        ...
        now=datetime.now(); ts=now.strftime("%Y-%m-%d %H:%M:%S"); sm=ts[11:16]
        print(f"{ts} | O₂: {o2v:.1f}% | Temp: {tv:.1f}°C | RH: {rhv:.1f}%")
        if time.time()-last_eink>=20:
            update_eink(sm,f"O₂: {o2v:.1f}%",f"RH: {rhv:.1f}%",f"Temp: {tv:.1f}°C", full=False)
            last_eink=time.time()
        ...
        if deviate>o2_thr:
            ...
            bodyA=(f"O₂ changed >{o2_thr:.1f}% from {o2_ref:.1f}% at {ts}.\n"
                   f"• O₂: {o2v:.1f}% | Temp: {tv:.1f}°C | RH: {rhv:.1f}%")
            if not alarm:
                ...
                send_email(subjA,bodyA,[logpath]); print(f"Alarm email sent at {ts}")
            ...
            if alarm:
                end=now; dur=end-start; maxd=max(deviated,key=lambda x:abs(x-o2_ref))
                with open(logpath,'a',newline='') as f: csv.writer(f).writerow([ts,sensor_id,f"{o2v:.1f}",f"{tv:.1f}",f"{rhv:.1f}"])
                with open(logpath,'a',newline='') as f: csv.writer(f).writerow([ts,sensor_id,'Alarm deactivated','',''])
                with open(alogpath,'a',newline='') as f: csv.writer(f).writerow([
                    sensor_id,
                ...
                bodyR=(f"O₂ restored ±{o2_thr:.1f}% at {ts}.\n"
                       f"• O₂: {o2v:.1f}% | Temp: {tv:.1f}°C | RH: {rhv:.1f}%")
                send_email(subjR,bodyR,[logpath,alogpath]); print(f"Restoration email at {ts}")
                alarm=False
        # logging
